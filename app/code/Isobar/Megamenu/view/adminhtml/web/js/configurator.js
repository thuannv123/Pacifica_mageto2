define(
    ['jquery', 'jquery/ui', "Magento_Ui/js/modal/modal", "mage/adminhtml/wysiwyg/widget"], function ($) {
        'use strict';
        ...
        $.widget(
            'tm.configurator', {
                ...
                _bind: function () {
                    ...
                    this.elements.addRowButton.on(
                        'click', function () {
                            self.addRow();
                        }
                    );
                    this.elements.content.on(
                        'click', '.mm_add_button', function (e) {
                            self.addColumnButtonClick(e);
                        }
                    );
                    this.elements.content.on(
                        'click', '.mm_remove_button', function (e) {
                            $(e.target).closest('.mm_row').remove();
                            self.updateData();
                        }
                    );
                    this.elements.content.on(
                        'click', '.mm_remove_column_button', function (e) {
                            $(e.target).closest('.mm_column').remove();
                            self.updateData();
                        }
                    );
                    this.elements.content.on(
                        'change', '.mm_column_width_select', function (e) {
                    ...
                    this.elements.content.on(
                        'click', '.add_from_select', function (e) {
                    ...
                    this.elements.content.on(
                        'click', '.mm_remove_selected', function (e) {
                    ...
                    this.elements.content.on(
                        'click', '.mm_add_products', function (e) {
                            self.addProducts(e);
                        }
                    );
                    this.elements.content.on(
                        'click', '.mm_add_widget', function (e) {
                            self.addWidget(e);
                        }
                    );
                    this.elements.content.on(
                        'click', '.mm_add_video', function (e) {
                            var multiselect = $(e.target).closest('.mm_column_inner').find('.mm_multiselect');
                    ...
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        for (var j = 0; j < columns.length; j++) {
                            ...
                            for (var k = 0; k < columnMultiselectOptions.length; k++) {
                                if($(columnMultiselectOptions[k]).text() == 'Widget') {
                                    var value = $(columnMultiselectOptions[k]).attr('value');
                ...
                hideElements: function () {
                    ...
                    var hideShow = function () {
                        ...
                        //$('#container div[data-index=mm_menu_type]').toggle();
                        if (self.options.level > 2) {
                            $('#container div[data-index=mm_menu_type]').hide();
                        }else {
                            $('#container div[data-index=mm_menu_type]').toggle();
                        }
                    };
                    if (turnOnOffSelect.val() == 0 && this.options.level !=1) {
                        hideShow();
                    }
                    turnOnOffSelect.change(hideShow);
                    if (this.options.level > 2) {
                        $('#container div[data-index=mm_menu_type]').hide();
                        $('#container div[data-index=mm_display_on]').hide();
                        $('#container div[data-index=mm_width]').hide();
                    }
                ...
                addProducts: function (e) {
                    ...
                    var addProductsToMultiselect = function (products) {
                        ...
                        var ids = [];
                        $.each(
                            productsObject, function (key, value) {
                                ids.push(key);
                            }
                        );
                ...
                addToColumnMultiselect: function (column, _value, _text) {
                    column.find('.mm_multiselect').append(
                        $(
                            '<option>', {
                                value: _value,
                                text: _text
