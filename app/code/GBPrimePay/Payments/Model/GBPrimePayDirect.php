        ...
        try {
            ...
            if (count($customers) > 0) {
                foreach ($customers as $cus) {
                    if ($cus['customer_email'] === $order->getCustomerEmail()) {
                        $customer = $cus;
                    }
                }
            } else {
            ...
            if ($this->_config->getCanDebug()) {
                $this->gbprimepayLogger->debug("item //" . $exception->getMessage());
            }
 ...
 if($iniactive==1 && !empty($token_id)){
      ...
      $card = [
          ...
          "card" => $callback['card'],
            ...
            $infoInstance = $this->getInfoInstance();
            $AdditionalInfo = $infoInstance->getAdditionalInformation();
            ...
            $payment->setCcExpYear($card['card']['expirationYear']);
                  ...
                  if(!empty($additionalDataRef)){
                      ...
                      $this->_clearQuoteAdditionInfo($order->getQuoteId(),$order->getId(), "data", $additionalDataRef->tokenid, $additionalDataRef->transaction_form, $additionalDataRef->is_save);
                      $payment->unsAdditionalInformation('data');
            ...
            if ($this->_config->getCanDebug()) {
                $this->gbprimepayLogger->debug("error //" . $exception->getMessage());
            }
            throw new CouldNotSaveException(
                __('Something went wrong. Please try again!')
            );
...
$item = array(
    ...
    public function capture(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        try {
            ...
            if ($this->_config->getCanDebug()) {
                $this->gbprimepayLogger->debug("cap //" . $exception->getMessage());
            }
    ...
    public function _clearQuoteAdditionInfo($quoteid, $orderid, $datainfo, $_tokenid, $_transaction_form, $_is_save)
    {
        $quote = $this->quoteRepository->get($quoteid);
            if ($quote->getId()){
                if($datainfo == "data"){               
                    $v_tokenid = isset($_tokenid) ? $_tokenid : "";                  
                    $v_transaction_form = isset($_transaction_form) ? $_transaction_form : "";                  
                    $v_is_save = isset($_is_save) ? $_is_save : "";

                    $paymentQuote = $quote->getPayment();
                    $paymentQuote->unsAdditionalInformation('data');
                    // $paymentQuote->setAdditionalInformation('transaction_form', $v_transaction_form);

                    $infoInstance = $this->getInfoInstance();        
                    $infoInstance->unsAdditionalInformation('data');
                    $infoInstance->setAdditionalInformation('transaction_form', $v_transaction_form);
                }
            }
    }
