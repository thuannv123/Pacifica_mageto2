...
class Product extends \Firebear\ImportExport\Model\Export\Product
{
    ...
    protected $firebearExport;

    const INVENTORY_SOURCE_CODE = 'inventory_source_code';
    const PRODUCT_IS_SALABLE = 'product_is_salable';
    ...
    public function __construct(
        ...
        \Firebear\ImportExport\Model\Export\Product\AdditionalFieldsPool $additionalFieldsPool,
        SeparatorFormatterInterface $separatorFormatter,
        GetListInterface $historyList,
        \Magento\Catalog\Model\ResourceModel\Product\CollectionFactory $productCollectionFactory,
        AttributeRepository $attributeRepository,
        \Firebear\ImportExport\Model\Export $firebearExport,
        array $dateAttrCodes = []
    ) {
        ...
        $this->firebearExport = $firebearExport;
    ...
    public function getExportData()
    {
        ...
        try {
            ...
            $multirawData = $this->collectMultiplerawData($rawData);
    ...
    public function export()
    {
        ...
        if (
            !empty($this->_parameters[Processor::BEHAVIOR_DATA]['export_by_page']) &&
            ...
            if (!$isAllStoresExported) {
                ...
                if (
                    isset($this->_parameters[Processor::LAST_ENTITY_ID])
            ...
            while (true) {
                ...
                $entityCollection = $this->_prepareEntityCollection($entityCollection);
                $this->paginateCollection($page, $this->getItemsPerPage());

                $entitiesCount = $entityCollection->getSize();
                if ($entitiesCount == 0) {
                    $this->firebearExport->addLogComment([__('There is no data for the export.')]);
                    break;
                }
        ...
        if (empty($counts)) {
            return $this->firebearExport->addLogComment([__('There is no data for the export.')]);
        }
    ...
    {
        if (
            !isset($this->_parameters[Export::FILTER_ELEMENT_GROUP])
            ...
            foreach ($this->filterAttributeCollection($this->getAttributeCollection()) as $attribute) {
                ...
                if (isset($exportFilter[$attrCode])) {
                    ...
                    if (
                        Export::FILTER_TYPE_MULTISELECT == $attrFilterType &&
                    ...
                    if (
                        Export::FILTER_TYPE_SELECT == $attrFilterType
                        && is_scalar($exportFilter[$attrCode])
                        && (int)$exportFilter[$attrCode] >= 0
                    ) {
                        if ($attrCode == self::INVENTORY_SOURCE_CODE) {
                            if (isset($exportFilter[self::INVENTORY_SOURCE_CODE])) {
                                $sourceCode = $exportFilter[self::INVENTORY_SOURCE_CODE];
                                        ...
                                        if (!empty($websiteId)) {
                                            $collection->getSelect()
                                                ->join(
                                                    ...
                                                    "product_website.product_id = e.entity_id AND product_website.website_id = $websiteId"
                                                );
                                        }
                                    }
                                }
                                if (isset($exportFilter['quantity_and_stock_status'])) {
                                    $inStock = (int)$exportFilter['quantity_and_stock_status'];
                                    $collection->getSelect()
                                        ->join(
                                            ['isi' => $collection->getTable('inventory_source_item')],
                                            'e.sku = isi.sku',
                                            ['source_code', 'status']
                                        )
                                        ->where('isi.status = ?', $inStock)
                                        ->where('isi.source_code = ?', $sourceCode);
                                } else {
                                    $collection->getSelect()
                                        ->join(
                                            ['isi' => $collection->getTable('inventory_source_item')],
                        ...
                        } else if ($attrCode == self::PRODUCT_IS_SALABLE) {
                            if (isset($exportFilter[self::PRODUCT_IS_SALABLE])) {
                                $stores = $this->getStores();
                                foreach ($stores as $storeId => $storeName) {
                                    $store = $this->_storeManager->getStore($storeId);
                                    if (!empty($store)) {
                                        ...
                                        if (!empty($websiteId)) {
                                            ...
                                            $tableName = 'inventory_stock_' . $websiteId;
                                            $collection
                                                ->getSelect()
                                                ->join(
                                                    ['isc' => $collection->getTable($tableName)],
                                                    'e.sku = isc.sku',
                                                    ['isc.is_salable']
                                                )
                                                ->where(
                                                    'isc.is_salable = ?',
                                                    (int)$exportFilter[self::PRODUCT_IS_SALABLE]
                                                );
                                        }
                                    }
                                }
        ...
        $collection = $this->_magentoPrepareEntityCollection($collection);
        ...
        if (
            isset($exportFilter['category_ids']) &&
    ...
    protected function collectMultiplerawData($rawData)
    {
        $productIds = array_keys($rawData);
        $rowWebsites = [];
        ...
        foreach ($productIds as $productId) {
            ...
            $productCollection->addCategoryIds()->addWebsiteNamesToResult();
            $this->addCategoryPosition($productCollection);
    ...
    protected function addAdditionalFields(array $rows): array
    {
        foreach ($this->additionalFieldsPool->getEntities() as $keyEntity => $entity) {
            if (
                $entity instanceof Msi && empty($this->_parameters[Processor::BEHAVIOR_DATA]['import_msi'])
            ) {
                continue;
            }
    ...
    protected function getCustomOptionsData($productIds)
    {
        ...
        foreach (array_keys($this->_storeIdToCode) as $storeId) {
            ...
            foreach ($options as $option) {
                ...
                if (
                    array_key_exists('max_characters', $optionData)
    ...
    private function getOptionValue($optionName, $defaultOptionsData, $optionData)
    {
        ...
        if (
            array_key_exists($optionId, $defaultOptionsData)
        ...
        foreach ($items as $itemId => $itemByStore) {
            ...
            foreach ($stores as $storeId => $storeCode) {
                ...
                foreach ($exportAttrCodes as $attrCodes) {
                    ...
                    if (
                        isset($this->_attributeTypes[$attrCodes]) &&
                    ...
                    if (
                        isset($this->attributeStoreValues[$attrCodes][$storeId][$attrValue])
                    ...
                    if ($this->_attributeTypes[$attrCodes] == 'datetime') {
                        if (
                            in_array($attrCodes, $this->dateAttrCodes) ||
                ...
                $data[$itemId][$storeId]['special_price'] = $item->getSpecialPrice() ?: "0";

                if ($item->getSpecialFromDate()) {
                    $formattedDateFrom = $this->formatDateWithOffset($item->getSpecialFromDate());
                    $data[$itemId][$storeId]['special_price_from_date'] = $formattedDateFrom;
                }

                if ($item->getSpecialToDate()) {
                    $formattedDateTo = $this->formatDateWithOffset($item->getSpecialToDate());
                    $data[$itemId][$storeId]['special_price_to_date'] = $formattedDateTo;
                }

                if (!empty($item->getTypeId()) && $item->getTypeId() == 'configurable') {
                    $data[$itemId][$storeId]['price'] = $item->getFinalPrice() ?: "0";
                }

                if (!empty($item->getColor())) {
                    $data[$itemId][$storeId]['color_id'] = $item->getColor();
                }
            }
        }
    ...
    protected function updatePageByPageExportStatus()
    {
        ...
        if (
            isset($this->getParameters()['only_admin'])
    ...
    protected function _getExportAttrCodes()
    {
        if (null === self::$attrCodes) {
            ...
            if (
                isset($parameters[Processor::ALL_FIELDS]) && $parameters[Processor::ALL_FIELDS] &&
    ...
    protected function updateDataWithCategoryColumns(&$dataRow, &$rowCategories, $productId)
    {
        ...
        if (
            isset($this->_parameters[Processor::BEHAVIOR_DATA]['category_levels_separator'])
        ...
        if (
            isset($this->_parameters[Processor::BEHAVIOR_DATA]['categories_separator'])
    ...
    protected function getStores()
    {
        if (null === $this->stores) {
            $this->stores = [];
            if (
                isset($this->_parameters[Processor::BEHAVIOR_DATA]['store_ids'])
    ...
    {
        if (
            !isset($this->cachedSwatchOptions[$storeId])
    ...
    protected function _magentoPrepareEntityCollection(\Magento\Eav\Model\Entity\Collection\AbstractCollection $collection)
    {
        if (
            !isset(
        ...
        foreach ($this->filterAttributeCollection($this->getAttributeCollection()) as $attribute) {
            ...
            if (isset($exportFilter[$attrCode])) {
                ...
                if (\Magento\ImportExport\Model\Export::FILTER_TYPE_SELECT == $attrFilterType) {
                    if ($attrCode == self::INVENTORY_SOURCE_CODE || $attrCode == self::PRODUCT_IS_SALABLE) {
                    } else {
    ...
    public function formatDateWithOffset($dateTime)
    {
        $formattedDateTime = $this->_localeDate->date($dateTime)->format('Y-m-d\TH:i');
        $offset = $this->_localeDate->date($dateTime)->format('P');
