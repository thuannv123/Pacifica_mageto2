...
class PendingQrcredit extends \GBPrimePay\Payments\Controller\Checkout\PendingQrcredit
{
    ...
    public function execute()
    {
        try {
            ...
            if (($_orderId == $_getEntityId) && ($_getIncrementId == $_getOrderByEntityId)) {
                ...
                if ($ordercompletestatus != 0) {
                    ...
                    if ($this->_config->getCanDebug()) {
                        $this->log("----------Data sent to API for QrCredit----------");
                    ...
                    if ($amount) {
                        ...
                        if ($this->_config->getCanDebug()) {
                            $this->log("----------Data reponse to API for Barcode----------");
            ...
            if ($this->_config->getCanDebug()) {
                $this->gbprimepayLogger->debug("PendingQrcredit error //" . $exception->getMessage());
            }
            $this->cancelOrder();
    ...
    {
        $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/gbprimepay/qrcredit_request.log');
        $logger = new \Zend_Log();
