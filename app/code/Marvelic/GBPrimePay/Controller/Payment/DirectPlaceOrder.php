        ...
        try {
            ...
            if ($this->getRequest()->isAjax()) {
                ...
                if ($this->_config->getCanDebug()) {
                    $this->gbprimepayLogger->debug("Payment: //" . json_encode($payment));
                }
                $this->sendEmailCustomer($order);
                // 3-D Secure Payment
                $secured = $this->gbprimepayDirect->_secured($order->getPayment(), $order->getBaseGrandTotal());
                if ($this->_config->getCanDebug()) {
                    $this->gbprimepayLogger->debug("Info payment //" . json_encode($secured));
                }
                if ($secured['id']) {
                    ...
                    if ($secured['resultCode'] === '00') {
                        ...
                        if ($this->_config->getCanDebug()) {
                            $this->gbprimepayLogger->debug("payment success but not change status //" . json_encode($secured));
                        }
                        return $result->setData([
                            'success' => true
                        ]);
                    ...
                    if ($this->_config->getCanDebug()) {
                        $this->gbprimepayLogger->debug("DirectPlaceOrder error//");
                    }
            ...
            if ($this->_config->getCanDebug()) {
                $this->gbprimepayLogger->debug("DirectPlaceOrder error//" . $exception->getMessage());
            }
            $this->cancelOrder();
